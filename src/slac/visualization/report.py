...
from slac.core.result import CalibrationResult, MetricResult, TransformResult
from slac.core.transform_artifacts import transform_artifact_from_result
from slac.evaluation.evidence_summary import (
    evidence_cases_from_result,
    evidence_summaries_from_result,
)
from slac.evaluation.metric_families import (
    grade_counts,
...
<head>
  ...
  <title>slac Report - {escape(result.run.id)}</title>
  <style>
...
<main>
  ...
  <h2>Provenance</h2>
  <table>
    <tr><th>slac Version</th><td>{escape(result.run.slac_version)}</td></tr>
