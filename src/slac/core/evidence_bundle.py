...
from slac.core.assessment import AssessmentArtifact, assess_report_evidence
...
EVIDENCE_BUNDLE_SCHEMA_VERSION: Literal["slac.evidence_bundle/v0.1"] = (
...
class EvidenceBundleManifest(StrictModel):
    ...
    schema_version: Literal["slac.evidence_bundle/v0.1"] = (
...
class EvidenceBundleVerification(StrictModel):
    ...
    schema_version: Literal["slac.evidence_bundle.verification/v0.1"] = (
...
def load_evidence_bundle(path: str | Path) -> EvidenceBundleManifest:
    ...
    try:
        ...
        raise SlacError(f"invalid evidence bundle {bundle_path}: {exc}") from exc
