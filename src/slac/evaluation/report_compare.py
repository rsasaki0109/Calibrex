...
perturbed candidates, Koide-style adapter output, and native slac output"
in one artifact. `slac.evaluation.compare` only compares two results at a
...
REPORT_COMPARISON_SCHEMA_VERSION: Literal["slac.report_comparison/v0.1"] = (
...
class ReportComparison(StrictModel):
    """Machine-readable N-way comparison across labeled slac results."""

    schema_version: Literal["slac.report_comparison/v0.1"] = (
...
def compare_reports(
    ...
    *,
    ...
    the two-way `slac compare` command. When `reference_label` is given,
