...
from slac.core.report_artifacts import EvidenceSummaryItem
...
from slac.evaluation.metric_families import metric_family

COMPARISON_SCHEMA_VERSION: Literal["slac.comparison/v0.1"] = (
...
class ResultComparison(StrictModel):
    """Machine-readable comparison between two slac results."""

    schema_version: Literal["slac.comparison/v0.1"] = COMPARISON_SCHEMA_VERSION
    left: ComparisonSide
...
def compare_results(
    ...
    *,
    ...
    """Compare two slac result files without changing either result."""
...
def comparison_side(
    ...
    `slac.evaluation.report_compare`) can build per-entry summaries
