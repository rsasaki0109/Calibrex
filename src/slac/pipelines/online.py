...
used by the offline path (`slac.solvers.native_lidar_point_to_plane_solver`)
...
deterministic train/holdout split (reusing `slac.evaluation.holdout`),
and evaluates a holdout gate mirroring the pass/fail/inconclusive semantics of
`slac.core.assessment`. Only a batch whose gate PASSES is adopted (the
...
artifact (`slac.core.online_timeline`).
"""
...
from slac.core.exceptions import ConfigError, DatasetError
from slac.core.frames import FrameGraph, FrameNode
from slac.core.geometry import SE3, Vector3
from slac.core.io import write_mapping
...
from slac.core.provenance import git_commit, sha256_path
    ...
    *,
    ...
    for connection, timestamp_ns, data in iter_rosbag2_messages(bag_path, topics=topics):
        ...
        if topic == source_topic:
            ...
            if motion_compensated and scan_records:
                source_scans.append(
                    OnlineSourceScan(
...
def _build_result(
    *,
    ...
    odometry_track: OdometryTrack | None = None,
    source_scans: list[OnlineSourceScan] | None = None,
    odometry_source_info: OdometrySourceInfo | None = None,
) -> CalibrationResult:
...
def _run_id(project_name: str) -> str:
    ...
    slug = re.sub(r"[^a-zA-Z0-9_.-]+", "_", project_name).strip("_") or "slac"
