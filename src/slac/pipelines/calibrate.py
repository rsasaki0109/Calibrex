...
from slac.core.geometry import SE3, normalize_quaternion_xyzw
...
from slac.evaluation.radar import radar_velocity_metrics_from_result
from slac.evaluation.timing import timing_metrics_from_inspection
from slac.graph.problem import build_problem
from slac.solvers.base import SolverAdapterResult
from slac.solvers.koide_lidar_camera_solver import (
    ...
    for name, transform in sorted(adapter_result.transforms.items()):
        if name in result.transforms:
            ...
            result.transforms[name].provenance = _adapter_output_provenance(
                ...
                note="adapter output applied to slac output estimate",
            )
            applied.append(name)
