...
* Parse the bag magic, the bag-header record, connection records, and chunk
  ...
  ``slac[rosbag1-lz4]`` extra and raises a clear error when missing.
...
other dataset adapters in :mod:`slac.data`.
"""
...
class Rosbag1DatasetStats:
    """Bag-level statistics returned to ``slac inspect``."""
...
def summarize_rosbag1(
    ...
    *,
    ...
    """Summarize supported LiDAR topics in a ROS 1 bag for ``slac inspect``.
