...
class RadarEvaluationConfig(StrictModel):
    max_frames_per_sensor: int = Field(default=30, ge=2, le=10_000)
    use_dataset_reference: bool = Field(
        default=False,
        description="evaluate dataset reference Radar extrinsics instead of configured candidates",
    )
    min_holdout_frames: int = Field(default=5, ge=1)
    min_holdout_static_returns: int = Field(default=50, ge=1)
    min_static_fraction: float = Field(default=0.20, ge=0.0, le=1.0)
    min_yaw_sensitivity_rms_mps_per_rad: float = Field(default=0.57, ge=0.0)
    min_mandatory_detectable_fraction: float = Field(default=1.0, ge=0.0, le=1.0)
    max_holdout_median_abs_residual_mps: float | None = Field(default=None, gt=0.0)
    max_holdout_rmse_mps: float | None = Field(default=None, gt=0.0)
...
class CalibrationConfig(StrictModel):
    schema_version: Literal["slac.config/v0.1"] = CONFIG_SCHEMA_VERSION
