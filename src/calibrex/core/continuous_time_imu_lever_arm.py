"""Schema-valid continuous-time IMU lever-arm recovery evidence."""
...
CONTINUOUS_TIME_IMU_LEVER_ARM_SCHEMA_VERSION: Literal[
    "slac.continuous_time_imu_lever_arm/v0.1"
] = "slac.continuous_time_imu_lever_arm/v0.1"


class ContinuousTimeImuLeverArmProvenance(StrictModel):
...
class ContinuousTimeImuLeverArmArtifact(StrictModel):
    """Holdout and known-bad evidence for knot-path IMU lever-arm factors."""
    ...
    lever_arm_error_m: float = Field(ge=0.0)
    ...
    known_bad_lever_arm_body_m: tuple[float, float, float]
...
def continuous_time_imu_lever_arm_json_schema() -> dict[str, Any]:
    """Return the JSON schema for IMU lever-arm recovery artifacts."""

    return ContinuousTimeImuLeverArmArtifact.model_json_schema()
