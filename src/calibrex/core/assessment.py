...
FALSIFICATION_POLICY_ID = "slac.falsification.lidar_pair/v0.1"
...
class AssessmentArtifact(StrictModel):
    ...
    schema_version: Literal["slac.assessment/v0.1"] = ASSESSMENT_SCHEMA_VERSION
...
def assess_report_evidence(
    ...
    *,
    ...
    radar_protocol = _radar_lidar_protocol(evidence)
    active_policy = policy or (
        _radar_assessment_policy(radar_protocol)
        if radar_protocol is not None
        and _lidar_pair_protocol(evidence) is None
        and _lidar_camera_protocol(evidence) is None
        else AssessmentPolicy()
    )
    ...
    elif radar_protocol is not None:
        rules.extend(
            [
                _radar_protocol_declaration_rule(radar_protocol),
                _radar_policy_decision_rule(evidence),
            ]
        )
    else:
        rules.append(
...
def _radar_lidar_protocol(evidence: ReportEvidenceArtifact) -> EvidenceProtocolItem | None:
    for protocol in evidence.protocols:
        if protocol.family == "radar_lidar":
            return protocol
    return None


def _radar_assessment_policy(protocol: EvidenceProtocolItem) -> AssessmentPolicy:
    parameters: dict[str, AssessmentScalar] = {}
    raw_policy = protocol.parameters.get("policy")
...
def _radar_protocol_declaration_rule(
    protocol: EvidenceProtocolItem,
) -> AssessmentRuleResult:
    complete = bool(
        protocol.protocol_id
        and protocol.candidate_transform
        and protocol.transform_convention
        and protocol.known_bad_case_count == 4
    )
    return AssessmentRuleResult(
        rule_id="protocol_declared",
        status="pass" if complete else "inconclusive",
        reason=(
            "radar yaw evidence protocol declares transforms and four controls"
            if complete
            else "radar yaw evidence protocol declaration is incomplete"
        ),
        observed={
            "protocol_id": protocol.protocol_id,
            "candidate_transform": protocol.candidate_transform,
...
def _radar_policy_decision_rule(
    evidence: ReportEvidenceArtifact,
) -> AssessmentRuleResult:
    summary = _summary(evidence, family="radar_lidar", check="Decision Boundary")
    if summary is None:
        return AssessmentRuleResult(
            rule_id="radar_policy_decision",
            status="inconclusive",
            reason="no Radar-LiDAR yaw decision boundary is materialized",
            evidence_refs=["summaries"],
        )
    ...
    return AssessmentRuleResult(
        ...
        status=status,
        reason=(
            "materialized Radar-LiDAR yaw policy passed"
            if status == "pass"
            else (
                "materialized Radar-LiDAR yaw policy rejected the candidate"
                if status == "fail"
                else "materialized Radar-LiDAR yaw policy is inconclusive"
            )
        ),
        metric_ids=summary.metric_ids,
        observed={"summary_status": summary.status},
