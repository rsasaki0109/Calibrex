...
class ProbabilisticCameraLidarRefinementResult:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "method": "probabilistic_multiframe_refinement/v0.2",
...
class ProbabilisticCameraLidarRefiner:
    ...
    def solve(
        ...
        holdout_result = evaluate_probabilistic_camera_lidar_pose(
            holdout, transform, settings
        )
        ...
        return ProbabilisticCameraLidarRefinementResult(
            ...
            final_holdout_evaluation=holdout_result,
            trace=tuple(trace),
            options=settings,
        )
    ...
    for frame in frames:
        for item in frame.correspondences:
            ...
            if point[2] <= 1.0e-9:
                skipped += 1
                continue
            u, v = _project_pinhole(point, frame)
            if not (
