...
`src/calibrex/solvers/planar_board_lidar_camera_solver.py`. A second
"multi-plane" solver would duplicate the same geometry, so this research line
extends that implementation and keeps its public schema stable.
...
The output `T_camera0_lidar0` translation is approximately
`[0.0220, -0.1661, -0.0842] m`. The result is schema-valid, its evidence bundle
verifies the downloaded input digest, and the overall quality grade is PASS.
