...
slac treats Koide-style targetless LiDAR-camera calibration as an external
...
By default the command is not executed. slac only records whether it is
...
`result_path` may point to a slac-compatible transform file:
...
When the transform is present, slac converts it into the configured frame
...
slac also reports cross-modal evaluation scaffolding in the same result:
...
When these metrics exist, `slac calibrate` and
`slac visualize --export-html` write
...
`slac demo kitti-lidar-camera-evidence` runs this whole flow end to end on
...
requires the official login-gated download flow, so slac cannot fetch it
