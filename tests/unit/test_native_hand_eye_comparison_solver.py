...
def test_chou_kamel_gap_gate_cannot_be_weakened_by_convergence(tmp_path: Path) -> None:
    archive = tmp_path / "robot_arm_w_color_camera_real.zip"
    _write_synthetic_archive(archive)
    config = _config(tmp_path, min_chou_gap=1.1)
    ...
    assert result.metrics["hand_eye_chou_kamel_rotation_nullspace_gap"].grade == "fail"
    assert result.status == "inconclusive"
    assert result.observability is not None
    assert result.observability.grade == "fail"
